...
![面试题1](images/面试题1.png)
![面试题1](images/面试题1_2.png)
...
1. 一共有几种实现方式？请分别用代码实现，并说明各个实现方式的优点和缺点?
2. 单例设计模式一共有8种写法(后面会依次说明)
   1. 饿汉式 两种
   2. 懒汉式 三种
   3. 双重检查
   4. 静态内部类
...
2. 实现方案1 :直接依赖了细节
   ...
   /**
    * 1. 简单，比较容易想到
    * 2. 如果获取的信息来源是 微信，短信等等， 则新增类， 同时 Perons 也要增加相应的接收方法
    * 3. 解决思路： 引入一个抽象的接口 IReceiver, 表示接收者, 这样 Person 类与接口 IReceiver 发生依赖
    *    因为 Email, WeiXin 等等属于接收的范围， 他们各自实现 IReceiver 接口就 ok, 这样就符合依赖倒转原则
    */
   class Person{
...
3. setter方式传递
   ...
   interface IOpenAndClose {
       public void open(); // 抽象方法
       //通过setter传递依赖，即open执行之前，一定要执行此set方法
   ...
   class ChangHong implements ITV {
       @Override
       public void play() {
           System.out.println("Changhong TV, turn on");
       }
   }
   ```
...
> 个人认为，开发中的继承关系是难免的，继承本来也是面向对象的一个特性，不应该从本来就具有继承关系的类上去拆分继承关系方面去考虑，而是应该考虑如果子类需要重写父类的方法，那么可以把父类的此方法定义为抽象方法，此类为抽象类，让子类自己去实现该方法即可。
> 或者，在开发中，如果有父子类关系，那么子类只能进行增强方法，而绝不允许有重写方法
...
5. 直接的朋友：每个对象都会与其他对象有耦合关系，只要两个对象之间有耦合关系，我们就说这两个对象之间是朋友关系。耦合的方式很多，依赖，关联，组合，聚合等。其中，我们称出现成员变量，方法参数，方法返回值中的类为直接的朋友，而
...
* 合成复用Composite Reuse Principle原则是尽量使用合成/聚合的方式，而不是使用继承

* 如果一个类A和类B有耦合关系，比如类B的方法中需要调用类A的方法，如果仅仅只是需要调用方法，而这两个类并不符合设计上的父子类关系，那就不应该让它们建立父子关系
...
3. 画UML图与写文章差不多，都是把自己的思想描述给别人看，关键在于思路和条理
...
### 3.5 实现关系（Implementation）

实现关系实际上就是A类实现B接口， 他是依赖关系的特例

### 3.6 关联关系（Association）
...
### 3.7 聚合关系（Aggregation）

聚合关系（Aggregation）表示的是整体和部分的关系， 整体与部分可以分开。 聚合关系是关联关系的特例，所以他具有关联的导航性与多重性。
如：一台电脑由键盘(keyboard)、显示器(monitor)，鼠标等组成；组成电脑的各个配件是可以从电脑上分离出来的， 使用带空心菱形的实线来表示
> 聚合关系在类中往往是使用set来进行依赖，而不是必须的(即类在创建实例时，并不是必须要有该依赖)

### 3.8 组合关系（Composition）

组合关系：也是整体与部分的关系，但是整体与部分不可以分开。

再看一个案例：在程序中定义实体： Person与IDCard、 Head, 那么 Head 和Person 就是 组合， IDCard 和 Person 就是聚合。
...
public class Person{
    ...
    private Head head = new Head();//组合。当然也能在Person的构造器中完成组合

    public setCard(IDCard card){
        this.card=card;
    }
}
public class IDCard{
...
3. 第3层： 学习过了设计模式，发现自己已经在使用了，并且发现了一些新的模式
挺好用的
4. 第4层： 阅读了很多别人写的源码和框架， 在其中看到别人设计模式，并且能够
领会设计模式的精妙和带来的好处。
5. 第5层： 代码写着写着，自己都没有意识到使用了设计模式，并且熟练的写了出
来。
...
4. 代码实现
   ...
   class Singleton{
       ...
       //2. 提供一个静态私有常量，在类加载时直接创建一个对象赋值给这个静态私有常量
...
> 因为JVM在装载类的时候是线程安全的，所以饿汉式的写法是天然的线程安全，但是没有做到懒加载
> 如果开发者确信这个类会被加载，且只会被加载一次，加载时机也是确定的，则饿汉式也是推荐的写法
...
> 类在加载时，静态变量以及静态代码块是一起组装到\<init\>方法中一起执行的，所以这两种写法本质是没有区别的
...
2. 但是这种同步并不能起到线程同步的作用。跟第3种实现方式遇到的情形一致，假如一个线程进入了`if (instance == null)`判断语句块，还未来得及往下执行，另一个线程也通过了这个判断语句，这时便会产生多个实例
...
class Singleton {
    ...
    public static Singleton getInstance() {
        //在同步代码块外面检查，防止实例创建后每次都进入同步块中，这样会降低效率
        if (instance == null) {
            synchronized (Singleton.class) {
                //在同步块内部，创建对象的时候，进行判断，防止最开始没有创建对象的时候，会有多个线程进来创建实例
                if(instance==null){
                    instance = new Singleton();
                }
            }
...
> 类加载器加载类只会加载一次(在类不存在时)
> 内部类的加载时机与外部类不同，内部类只有在使用的时候才会被加载
> JVM在装载类的时候是线程安全的
...
![单例模式Runtime](images/单例模式Runtime.png)
...
> 上面的代码，在生产Pizza时，需要使用`if...else`进行判断Pizza的种类，如果添加了Pizza的种类，则`if...else`的代码就需要进行修改了。如果有多处代码都进行了生产Pizza，那么每当修改了Pizza的种类，则多处的代码都需要修改

传统的方式的优缺点

1. 优点是比较好理解，简单易操作。
2. 绝对是违反了设计模式的ocp原则，即对扩展开放，对修改关闭。即当我们给类增加新功能的时候，尽量不修改代码，或者可行少修改代码。
3. 比如我们这时要新增加一个Pizza的种类（Pepper披萨），我们需要做如下修改。
如果我们增加一个Pizza 类，只要是订购Pizza的代码都需要修改。
4. 改进的思路分析
   * 分析：修改代码可以接受，但是如果我们在其他的地方也有创建Pizza的代码，就意味着，也需要修改，而创建Pizza的代码，经常有多处。
   * 想法：把创建比萨对象封装到一个类中，这样我们有新的披萨种类时，只需要修改该类就可，其他有创造到披萨对象的代码就不需要修改了.->简单工厂模式

### 6.2 简单工厂模式

1. 简单工厂模式是属于创造型模式，是工厂模式的一种。简单工厂模式是由一个工厂对象决定创建出哪一种产品类的实例。简单工厂模式是工厂模式家族中最简单实用的模式
2. 简单工厂模式：定义了一个创建对象的类，由这个类来封装实例化对象的行为（代码）
3. 在软件开发中，当我们会用到大量的创造某种，某类或者某批对象时，就会使用到工厂模式

使用简单工厂模式

1. 简单工厂模式的设计方案: 定义一个可以实例化Pizza对象的类，封装创建对象的代
码。
...
修改后，不管开发的时候，哪个地方需要生产Pizza，都只需要调用SimpleFactory类的createPizza方法，而且如果有新增的Pizza，则只需要在新增Pizza类后修改SimpleFactory类的逻辑，而其他的业务代码都不需要进行修改

简单工厂模式还可以将工厂类的创建实例的方法定义成静态方法
代码如下：
...
将上面的抽象方法模式代码进行修改，关于Pizza的定义不需要修改，现在就是需要创建三个工厂，其中一个是抽象的工厂接口，另外两个是抽象工厂的实现类。然后OrderPizza类只需要在创建Pizza订单的时候，依赖传入的工厂类来进行Pizza的创建
...
1. 简单工厂模式实现了将客户端和生成Pizza类的代码进行分离，在工厂类中可以添加所需的Pizza的逻辑代码。但是这不符合"开发-闭合"原则，也就是说对扩展开发，对修改关闭。目前简单工厂模式每次在新增Pizza种类时，就需要修改工厂类的创建实例的代码，增加if-else判断
2. 工厂方法将生成Pizza的时机延迟，通过对应的工厂类来生成对于的Pizza类，即无论增加多少Pizza种类，都只需要增加对应的工厂类来实现。但是如果Pizza种类过多，就会生成很多工厂类。这也不是很方便
3. 抽象工厂模式中可以定义实现不止一个工厂接口，一个工厂也可以生成不止一个产品类，抽象工厂模式将Pizza进行分类，然后每一大类的Pizza种类对应一个工厂实现

### 6.5 工厂模式在JDK-Calendar 应用
...
> 上面的类所对应的对象在克隆的时候，其属性如果是基本数据类型和String类型，都会被拷贝，且实例对象和拷贝所得的对象，它们的内存地址空间是不同的。但是如果类中的属性为引用型属性，则其属性其实仍是地址复制
...
1. 使用 重写clone方法实现深拷贝
   ...
   public class Client {
       ...
       public static void main(String[] args) throws Exception {
           ...
           System.out.println("p2.name=" + p.name + "p2.deepCloneableTarget=" + p1.deepCloneableTarget.hashCode());
       }
   }
   ```

2. 使用序列化来实现深拷贝。只需要修改被克隆的类中的方法，提供一个用于克隆的方法，该方法体内使用序列化使用对属性，无论是基本类型还是引用类型，进行克隆
...
![建造者模式示例原理](images/建造者模式示例原理.png)
...
2. 原始码中建构者模式角色分析
   ...
   * String AbstractStringBuilder实现了可附加的接口方法，这里的AbstractStringBuilder已经是建造者，只是不能实例化
   * StringBuilder即成为了指挥者角色，同时并入了具体的建造者，建造方法的实现是由AbstractStringBuilder完成，而StringBuilder继承了AbstractStringBuilder
...
5. 建造者模式所创建的产品一般具有较多的共同点，其组成部分相似， 如果**产品之间
...
基本介绍： Adapter类，通过继承 src(被继承，源)类，实现 dst(目标) 类接口，完成src->dst的适配。

类适配器模式应用实例

1. 应用实例说明
以生活中充电器的例子来讲解适配器，充电器本身相当于Adapter， 220V交流电相当于src (即被适配者)， 我们的目dst(即 目标)是5V直流电。因为手机只能直接使用5V的电压充电，使用适配器后，手机是使用充5V的方式进行充电，只不过接了适配器，其实真正在使用的还是220V，但是经过转换后让手机能充上5V的电
...
public class Client {
    ...
    public static void main(String[] args) {
        ...
        phone.charging(new VoltageAdapter());//手机使用适配器充电
    }

}
```

> 适配器继承了一个类，此类是充220V电压，而实现的是充5V电压的接口，手机充电调用的是充5V电压的接口方法，只不过真正执行方法时，是调用的适配器实现的该方法，该方法对200V电压->5V电压之间进行了转换
...
只需要修改适配器类，将原先适配器“继承被适配类，实现适配接口”，修改成“聚合依赖被适配类，然后实现适配接口”
...
2. 有时候我们不想实现
Animator.AnimatorListener接口的全部方法，只想监听onAnimationStart，会如下写：
...
3. AnimatorListenerAdapter类，就是一个接口适配器，它空实现了Animator.AnimatorListener类(src)的所有方法.
4. AnimatorListener是一个接口

### 9.6 适配器模式在SpringMVC框架应用
...
> 上面对于具体的手机而言，有两个维度，一个是款式，一个是品牌，这两个维度可以相互组合成一个具体的手机。如果让它们从一个维度扩展，然后再向下扩展，则会如上图所示，类爆炸。
> 可以将这两个维度分开扩展，并且每个维度最好有一个抽象类，然后这两个维度聚合的时候，使用父类聚合，这样就不会出现一个维度A在扩展的时候，另一个维度B对应的所有值都需要扩展这个维度A的变化
...
从代码可以看到，有两个抽象类：Phone和Brand，分别表示手机款式和手机品牌，然后将Brand类作为属性聚合在Phone的类中。现在如果需要添加一个手机款式，直接写一个子类继承Phone即可，不用考虑品牌；同样的，如果需要添加一个手机品牌，直接写一个子类继承Brand即可，不用考虑手机款式。

### 10.3桥接模式在JDBC的应用

Jdbc 的 Driver接口，如果从桥接模式来看， Driver就是一个接口，下面可以有MySQL的Driver， Oracle的Driver，这些就可以当做实现接口类
在Java中通过Connection提供给各个数据库一样的操作接口，Connection可以看作抽象类。可以说用来操作不同数据库的方法都是相同的，不过MySQL有自己的ConnectionImpl类，同样Oracle也有对应的实现类。这里Driver和Connection之间是通过DriverManager类进行桥接的
...
先定义个Drink抽象类，然后让所有的饮品都继承该类，并实现cost()方法和desc()方法，如果有饮品组合，也定义一个组合的新类。。。

方案1-解决星巴克咖啡订单问题分析

1. Drink 是一个抽象类，表示饮料
2. desc就是对咖啡的描述, 比如咖啡的名字
3. cost() 方法就是计算费用， Drink 类中做成一个抽象方法.
4. Decaf 就是单品咖啡， 继承Drink, 并实现cost
5. Espress && Milk 就是单品咖啡+调料， 这个组合很多
6. 问题：这样设计，会有很多类，当我们增加一个单品咖啡，或者一个新的调料，类的数量就会倍增，就会出现类爆炸。这近乎排列组合的形式编写类是不可行的方法

方案2-解决星巴克咖啡订单(好点)

* 前面分析到方案1因为咖啡单品+调料组合会造成类的倍增，因此可以做改进，将调料内置到Drink类，这样就不会造成类数量过多。从而提高项目的维护性
* 方案二就是将饮料和调料分开，然后让调料称为饮品的属性，就不会发生因排列组合而产生很多类
* 说明: milk,soy,chocolate 可以设计为Boolean,表示是否要添加相应的调料

方案2-的问题分析

1. 方案2可以控制类的数量，不至于造成很多的类
2. 考虑到用户可以添加多份 调料时，可以将调料的有无修改为调料的份数
3. 在增加或者删除调料种类时，代码的维护量很大
4. 调料价格改变时，需要改动现有代码
5. 以后可能会开发出新饮料。对这些饮料而言（例如：冰茶），某些调料可能并不适合
6. 一旦出现新的调料，我们就需要加上新的方法，并改变超类中的cost()方法。
...
public class CoffeeBar {
    ...
    public static void main(String[] args) {
        // 装饰者模式下的订单：2份巧克力+一份牛奶的无因咖啡

        // 1. 点一份 无因咖啡
        Drink order = new DeCaf();
        System.out.println("order 无因咖啡  费用 =" + order2.cost());
        System.out.println("order 无因咖啡 描述 = " + order2.getDes());
...
![HashMap组合模式](images/HashMap组合模式.png)
...
![外观模式-传统方法](images/外观模式-传统方法.png)
...
![外观模式的原理类图](images/外观模式的原理类图.png)

原理类图的说明(外观模式的角色)

1. 外观类(Facade): 为调用端提供统一的调用接口, 外观类知道哪些子系统负责处理请求,从而将调用端的请求理给适当子系统对象
...
1. 应用实例要求
   使用外观模式来完成家庭影院项目
2. 思路分析和图解(类图)
    ![外观模式-影院项目](images/外观模式-影院项目.png)
3. 代码实现
...
![享元模式类图](images/享元模式类图.png)

对原理图的说明-即（模式的角色及职责）

1. FlyWeight是抽象的享元角色，他是产品的抽象类，同时定义出对象的外部状态和内部状态（后面介绍）的接口
或实现
2. ConcreteFlyWeight是具体的享元角色，是具体的产品类，实现抽象角色定义相关业务
3. UnSharedConcreteFlyWeight是不可共享的角色，一般不会出现在享元工厂。
4. FlyWeightFactory享元工厂类，使用内置一个池容器（集合），同时提供从池中获取对象方法
...
### 13.5 享元模式在Integer的应用

Integer将0-127之间的数据进行了缓存，将这些数据共享
...
1. 定义一个被代理类的父接口Person
2. 目标对象Student实现接口Person
3. 使用静态代理方式,就需要在代理对象StudentProxy中也实现Person
...
public class ProxyTest {
    ...
    public void dynamicProxyTest(){
        Student student=new Student();
        Student instance=(Student) new ProxyFactory(student).getProxyInstance();
...
1. 在模板方法模式的父类中，可以定义一个方法，它默认不做任何事，子类可以视情况要不要覆盖它，该方法称为“钩子”

### 15.4 模板方法模式在Spring框架应用

Spring IOC实现的AbstractApplicationContext中实现了许多空方法
...
![JdbcTemplate](images/JdbcTemplate.png)

这并不是严格上的格式来实现的命令模式，调度者和命令的实现是在一个类中实现的，命令实例是以内部类来实现的，省去了单独写一个类来继承抽象的命令类，而在doInPreparedStatement方法中去调用了接受者即原生的PreparedStatement对象来完成数据库查询
...
2. 传统方案
先定义一个Person类，然后写两个类Man,Woman来继承这个Person,实现投票，但是由于每个类别都可以投成功或失败，即要实现两个方法，表示投成功，以及投失败

传统方式的问题分析

1. 如果系统比较小，还是ok的，但是考虑系统增加越来越多新的设计需求时，对代码改
动较大，违反了ocp原则， 不利于维护
2. 扩展性不好，比如 增加了 新的观众类型，或者评价，都不好做
...
![访问者模式原理类图](images/访问者模式原理类图.png)
...
看一个具体的需求
编写程序展示一个学校院系结构：需求是这样，要在一个页面中展示出学校的院系组成，一个学校有多个学院，一个学院有多个系。 如下(计算机学院的信息是使用数组存储，信息工程学院的信息是使用集合存储)：
...
![迭代器模式的原理类图](images/迭代器模式的原理类图.png)
...
### 18.5 迭代器模式在JDK-ArrayList集合应用

1. JDK的ArrayList 集合中就使用了迭代器模式
![ArrayList的迭代器](images/ArrayList的迭代器.png)
2. 角色分析说明
...
1. 定义一个天气预报的发布类，通过这个类可以获取到天气预报信息
2. 定义一个第三方类，该第三方想通过天气预报类来获取天气信息，而它是直接与天气预报类进行聚合，当天气预报类的天气信息更新时，会调用聚合的属性的update方法，将更新的天气信息发布到该第三方类中

具体实现：

```java
/**
 * 显示当前天气情况（可以理解成是第三方气象网自己的网站）
 */
...
* Observer：接收输入
* 观察者模式：对象之间多对一依赖的一种设计方案，被依赖的对象为Subject，依赖的对象为Observer， Subject通知Observer变化,比如这里的报社是Subject，是1的一方。读者是Observer，是多的一方。

观察者模式解决天气预报需求

![观察者模式解决天气预报原理](images/观察者模式解决天气预报原理.png)
...
### 19.3 观察者模式在Jdk应用的源码分析

![jdk中的观察者](images/jdk中的观察者.png)

1. Jdk的Observable类就使用了观察者模式，相当于该模式中的Subject
2. 而Observer类则就是观察者
3. 模式角色分析
   Observable 的作用和地位等价于Subject
   Observable 是类，不是接口，类中已经实现了核心的方法 ,即管理Observer的方法 add.. delete .. notify...
   Observer 的作用和地位等价于 Observer, 有update
   Observable 和 Observer 的使用方法与上面的模式一样，只是Observable 是类，通过继承来实现观察者模式
...
4. ConcreteColleague具体的同事类，会有很多，每个同事只知道自己的行为，而不是了解其他同事类的行为（方法）,但是他们都依赖中介者对象
...
2. 思路分析和图解(类图)
![mediator2](images/mediator2.png)
...
利于管理，开销也很大.
2. 传统的方式是简单地做备份， new出另外一个对象出来，再把需要备份的数据放到
这个新对象，但这就暴露了对象内部的细节

### 21.2 备忘录模式基本介绍

1. 备忘录模式（Memento Pattern） 在不破坏封装性的前提下，捕获一个对象的内部状态，并在该对象之外保存这个状态。这样以后就可将该对象恢复到原先保的状态
...
4. 说明：如果希望保存多个originator对象的不同时间的状态，也可以，只需要使用 `HashMap <String, List<>>`
...
3. 如果类的成员变量过多，势必会占用比较大的资源，而且每一次保存都会消耗一定内存, 这个需要注意
4. 适用的应用场景：1、后悔药;2、打游戏时的存档;3、Windows 里的 ctrl+ z;IE 中的后退;4、数据库的事务管理
...
通过解释器模式来实现四则运算，如计算a+b-c+d-e的值，具体要求
...
![解释器模式原理类图](images/解释器模式原理类图.png)
...
### 22.4 解释器模式在Spring框架应用

1. Spring框架中SpelExpressionParser使用到了解释器模式
   ...
   public class MyTest {
       ...
       public void  test(){
           ...
           Expression expression =parser.parseExpression("12*(4-1)/2+23");
...
2. 说明
![Spring中的解释器](images/Spring中的解释器.png)
...
传统解决的思路就是定义一个鸭子父类，并且定义各种行为方法，然后让子类继承该类后，重写方法
...
4. 问题又来了，如果我们有一个玩具鸭子ToyDuck, 这样就需要ToyDuck去覆盖Duck的所有实现的方法
...
### 24.4 策略模式在JDK-Arrays 应用

1. JDK的 Arrays 的Comparator就使用了策略模式
`public static <T> void sort(T[] a, Comparator<? super T> c)`
将比较的策略通过一个接口实现类来传入，将类的某些功能分开，而不是让Arrays去实现
...
开发者可以独立于其Context改变它，使它易于切换、易于理解、易于扩展
...
5. 如果金额 超过30000以上，有校长审批 ( x>30000)
请设计程序完成采购审批项目

传统方案解决OA系统审批

![职责链传统](images/职责链传统.png)
...
![职责链介绍](images/职责链介绍.png)

### 25.3 职责链模式的原理类图

对原理类图的说明-即(职责链模式的角色及职责)

![职责链原理类图](images/职责链原理类图.png)
...
3. Request里有很多属性，表示一个请求

### 25.4 职责链模式解决OA系统采购审批
...
1. 有一点需要说明：设计模式是程序员在编程中， 有意或者是无意使用到的(也不是所有的程序员都学习过设计模式)，并且同一种设计模式实现方式也不是100%的一样，设计模式主要是提高程序的扩展性，可读性，可维护性、规范性。
2. 所以在看某个设计模式在源码框架中使用时，和标准的设计模式写法可能会有些出入，比如组合模式 Component 可以是抽象类，接口，也可以是一个实现类， 源码(JDK HashMap源码)， Component 就可能不一样
3. 对于框架源码，源码中部分使用了A设计模式，还部分使用了B设计模式，也是有可能的，也就是说设计模式是可以结合使用的
4. 因为设计模式主要是一种编程思想，既然是思想，具体实现方式，就不可能100%的一样(当然，程序的设计结构基本是一样的)
5. 所以，提醒大家我们学习设计模式时，（包括看源码分析），要抓住本质，就是使用这个设计模式到底带了了什么好处？ 是 扩展性提高了，还是更加规范了， 这样我们才能领会设计模式的精妙之处
