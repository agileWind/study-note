...
* Java程序在执行中所发生的异常事件可分两类：
  ...
  * Exception：其他因编程错误或偶然的外在因素导致的一般性问题，可以使用针对性的代码进行处理，如：
    1. 空指针访问异常，NullPointerException，比如定义二维数组是只初始化了一维数组，却直接访问二维的元素
    2. 试图读取不存在的文件，这是非运行时异常，即受检异常，FileNotFoundException
    3. 类型转换异常，ClassCastException，比如父类引用指向子类对象，然后被强转成了其他子类
    4. 数组角标越界异常，ArrayIndexOutOfBoundsException
    5. 操作数据库异常：SQLException
    6. 输入输出异常：IOException
    7. 算数异常类：ArithmeticException，比如除0
    8. 数字格式化异常：NumberFormatException，比如包装类的数据相互转换
    9. 找不到方法异常：NoSuchMethodException，反射执行方法时出现的异常
